...
async function handleDispatchCampaign(job) {
  try {
    ...
    // whatsapp-web.js usa wbot.info em vez de wbot.user
    if (!wbot?.info?.wid) {
      logger.error(`campaignQueue -> DispatchCampaign -> error: wbot info not found`);
      return;
    }
    ...
    if (campaign.mediaPath) {
      ...
      // whatsapp-web.js usa sendMessage(chatId, content) diretamente
