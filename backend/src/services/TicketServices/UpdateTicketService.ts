...
const UpdateTicketService = async ({
  ...
  try {
    ...
    if (status !== undefined && ["closed"].indexOf(status) > -1) {
      ...
      // Reset do sistema de semáforo quando ticket é fechado
      try {
        await MessageSemaphoreService.resetTicketSemaphore(ticket.id);
      } catch (error) {
        console.error('Erro ao resetar semáforo do ticket:', error);
      }

      // Registrar no histórico de tickets fechados
      try {
        ...
        console.error('Erro ao registrar histórico do ticket fechado:', error);
      }
    ...
    if (settingsTransfTicket?.value === "enabled") {
      ...
      if (oldQueueId !== queueId && oldUserId === userId && !isNil(oldQueueId) && !isNil(queueId)) {
        ...
        // whatsapp-web.js usa formato direto: sendMessage(chatId, content)
        ...
        if (oldUserId !== userId && oldQueueId === queueId && !isNil(oldUserId) && !isNil(userId)) {
          ...
          if (oldUserId !== userId && !isNil(oldUserId) && !isNil(userId) && oldQueueId !== queueId && !isNil(oldQueueId) && !isNil(queueId)) {
            ...
            // whatsapp-web.js usa formato direto
            const queueChangedMessage = await wbot.sendMessage(
              `${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`,
              translatedMessage[language]
            );
