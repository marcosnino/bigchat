...
const MessagesList = ({ ticket, ticketId, isGroup }) => {
  ...
  useEffect(() => {
    ...
    socket.on(`company-${companyId}-appMessage`, (data) => {
      ...
      if (data.action === "clearHistory" && data.ticketId === currentTicketId.current) {
        dispatch({ type: "RESET" });
        setPageNumber(1);
        setHasMore(false);
      }
  ...
  const checkMessageMedia = (message) => {
    ...
    /*else if (message.mediaType === "multi_vcard") {
      ...
      return (
        <audio controls preload="auto" className={classes.audioMedia}>
          <source src={message.mediaUrl} type="audio/ogg"></source>
          <source src={message.mediaUrl} type="audio/mpeg"></source>
          <source src={message.mediaUrl} type="audio/mp4"></source>
          <source src={message.mediaUrl} type="audio/wav"></source>
          O navegador não suporta reprodução de áudio.
        </audio>
      );
            ...
            && (
              <div className={classes.downloadMedia}>
                <audio controls preload="auto">
                  ...
                  <source src={message.quotedMsg.mediaUrl} type="audio/mpeg"></source>
                  <source src={message.quotedMsg.mediaUrl} type="audio/mp4"></source>
                  O navegador não suporta reprodução de áudio.
                </audio>
              </div>
            )
