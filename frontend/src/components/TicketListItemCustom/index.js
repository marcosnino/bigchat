...
const TicketListItemCustom = ({ ticket }) => {
  ...
  const handleContextMenuAcceptTicket = async () => {
    if (contextMenu && contextMenu.status === "pending") {
      await handleAcepptTicket(contextMenu.id);
    }
    handleContextMenuClose();
  };
  ...
  return (
    <React.Fragment key={ticket.id}>
      ...
      >
        {contextMenu && contextMenu.status === "pending" && (
          <MenuItem onClick={handleContextMenuAcceptTicket}>
            {i18n.t("ticketsList.buttons.accept")}
          </MenuItem>
        )}
        {contextMenu && (contextMenu.status === "open" || contextMenu.status === "pending") && (
          <MenuItem onClick={handleContextMenuCloseTicket}>
            {i18n.t("ticketsList.buttons.closed")}
          </MenuItem>
        )}
      </Menu>
