...
export default function Options(props) {
  ...
  const [, setCheckMsgIsGroup] = useState(false); // eslint-disable-line no-unused-vars
  ...
  // AUTO ACEITAR TICKETS
  const [autoAcceptTickets, setAutoAcceptTickets] = useState("disabled");
  const [loadingAutoAcceptTickets, setLoadingAutoAcceptTickets] = useState(false);
  ...
  useEffect(() => {
    if (Array.isArray(settings) && settings.length) {
	  ...
	  {/*AUTO ACEITAR TICKETS*/}
      const autoAcceptTickets = settings.find((s) => s.key === "autoAcceptTickets");
      if (autoAcceptTickets) {
        setAutoAcceptTickets(autoAcceptTickets.value);
      }
  ...
  async function handleAutoAcceptTickets(value) {
    setAutoAcceptTickets(value);
    setLoadingAutoAcceptTickets(true);
    await update({
      key: "autoAcceptTickets", 
      value,
    });
    toast.success(i18n.t("settings.options.toasts.success"));
    setLoadingAutoAcceptTickets(false);
  }
  ...
  return (
    <>
      <Grid spacing={3} container>
        ...
        <Grid xs={12} sm={6} md={4} item>
          <FormControl className={classes.selectContainer}>
            <InputLabel id="autoAcceptTickets-label">
              {i18n.t("settings.options.fields.autoAcceptTickets.title")}
            </InputLabel>
            <Select
              labelId="autoAcceptTickets-label"
              value={autoAcceptTickets}
              onChange={async (e) => {
                handleAutoAcceptTickets(e.target.value);
              }}
            >
              <MenuItem value={"disabled"}>{i18n.t("settings.options.fields.disabled")}</MenuItem>
              <MenuItem value={"enabled"}>{i18n.t("settings.options.fields.enabled")}</MenuItem>
            ...
            <FormHelperText>
              {loadingAutoAcceptTickets && i18n.t("settings.options.updating")}
            </FormHelperText>
          </FormControl>
        </Grid>
		{/* ENVIAR SAUDAÇÃO AO ACEITAR O TICKET */}
